...
I have a [repo](https://github.com/darrylbalderas/public-projects/tree/main/demo-backstage) where I have gone through the steps above plus a couple of other things that will not be mentioned in this post like
...
2. Configure postgress database
    ...
    1. Copy this [`docker-compose.yaml`](https://github.com/darrylbalderas/public-projects/blob/main/demo-backstage/docker-compose.yaml) file to define a postgres DB e.g.
    ...
    1. In your `backstage-app/app-config.yaml` update [database section](https://github.com/darrylbalderas/public-projects/blob/main/demo-backstage/app-config.yaml#L30-L36) to point to the values for database e.g.
...
3. Configure AWS connection
    ...
    3. After you have the access id and key for your user, you can [update](https://github.com/darrylbalderas/public-projects/blob/main/demo-backstage/app-config.yaml#L51-L53) your [`backstage-app/app-config.yaml`](https://backstage.io/docs/integrations/aws-s3/locations#configuration) e.g.
...
5. Create simple [resource golang api](https://github.com/darrylbalderas/public-projects/tree/main/demo-backstage/backstage-infra/apis/resources) to used in dynamic fields in our software templates
...
6. Create custom software template that uses resource golang for dynamic fields. Here is an [example](https://github.com/darrylbalderas/public-projects/blob/main/demo-backstage/backstage-infra/catalog/scaffold-template-dynamic-field/template.yaml)
    ...
    3. Once you have created software template, make sure to register it in your `backstage-app/app-config.yaml`. Here is an [example](https://github.com/darrylbalderas/public-projects/blob/main/demo-backstage/app-config.yaml#L121-L124)
...
Takeways from Backstage is that as a platform engineer, you get into a mindset of how can I surface more information to our users so that they can make better informed decision. We start to enter into interface and custom plugin land where if folks need certain functionality, we could just introduce new APIs.
